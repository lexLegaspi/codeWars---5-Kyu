...
function movingShift(s, shift) {
    let sToArr = s.split('')
    let result = []
    let length = Math.ceil(s.length/5)
    let last = s.length-(Math.ceil(s.length/5)*4)
    ...
    if (ch >= 'a' && ch <= 'z') {
      ...
      sToArr[i] = String.fromCharCode((code + shift) % 26 + 97);
    } 
    else if (ch >= 'A' && ch <= 'Z') {
      let code = ch.charCodeAt(0) - 65;
      sToArr[i] = String.fromCharCode((code + shift) % 26 + 65);
     
    }
      shift++;
  }
    
  let equalPart = Math.ceil(sToArr.length/5)
  while(result.length != 5) {
    result.push(sToArr.splice(0,equalPart).join(''))
  }
  
  console.log(equalPart)
